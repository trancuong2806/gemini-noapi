...
# --- BẢN VÁ LỖI HIỂN THỊ RÁC TRÊN WINDOWS KHI ĐÓNG ỨNG DỤNG ---
# Ẩn lỗi "ValueError: I/O operation on closed pipe" của asyncio trên Windows
orig_unraisablehook = sys.unraisablehook
def silence_asyncio_pipe_error(unraisable):
    if issubclass(unraisable.exc_type, ValueError) and "I/O operation on closed pipe" in str(unraisable.exc_value):
        return
    orig_unraisablehook(unraisable)
sys.unraisablehook = silence_asyncio_pipe_error
# -------------------------------------------------------------
...
class GeminiClient:
    ...
    async def start_browser(self):
        ...
        if not self._is_port_open(port):
            ...
            # Khởi động hành trình quy trình độc lập của OS (Daemon)
    ...
    # ----------------------------------------------------------
    # Khối chức năng đăng nhập, mở web, tìm ô nhập
    # ----------------------------------------------------------
    ...
    async def minimize_window(self):
        if not self.browser or not getattr(self, "page", None): return
        try:
            import nodriver.cdp.browser as cdp_browser
            target_id = self.page.target.target_id
            res = await self.browser.connection.send(cdp_browser.get_window_for_target(target_id=target_id))
    ...
    async def login_google(self, email: str, password: str) -> bool:
        ...
        self.page = await self.page.get(self.LOGIN_URL)
        await self.human.random_delay(2, 4)

        try:
            email_input = await self.page.select('input[type="email"]')
            if not email_input: email_input = await self.page.find("Email or phone", best_match=True)
        ...
        try:
            ...
            if not pw_input: pw_input = await self.page.find("Enter your password", best_match=True)
        ...
        for _ in range(120):
            ...
            if "accounts.google.com" not in cur_url or "myaccount" in cur_url:
                break
    ...
    async def open_gemini(self) -> bool:
        target_url = self.config._data.get("last_chat_url", "") if self.config.save_last_chat_url else ""
        if self.config.guest_mode:
        ...
        self.page = await self.page.get(target_url)
        await self.human.random_delay(3, 5)
    ...
    (() => {
        ...
        for (let b of buttons) {
            if (b.disabled || b.getAttribute('aria-disabled') === 'true' || b.classList.contains('disabled')) continue;
            
            if (b.closest('model-response') || b.closest('message-content') || b.closest('.message-content') || b.closest('.chat-history')) continue;
    ...
    async def _find_input_area(self):
        sel = 'div.ql-editor[contenteditable="true"], .ql-editor, rich-textarea div[contenteditable="true"], div[contenteditable="true"][role="textbox"], textarea'
        try:
            el = await self.page.select(sel, timeout=2)
            if el: return el
        except Exception: pass
        
        for hint in ["Enter a prompt", "Nhập câu lệnh", "Nhập nội dung"]:
            try:
    ...
    async def _find_send_button(self):
        selectors = ['button[aria-label*="Send"]', 'button[aria-label*="send"]', 'button[aria-label*="Gửi"]', 'button[aria-label*="gửi"]', 'button.send-button']
    ...
    async def _paste_file(self, file_path: str) -> bool:
        path = Path(file_path)
        if not path.exists():
            return False
        ...
        await self.page.evaluate("window._fileChunks = [];")
        
        chunk_size = 1024 * 1024 
        for i in range(0, len(b64_data), chunk_size):
            chunk = b64_data[i:i+chunk_size]
            await self.page.evaluate(f"window._fileChunks.push('{chunk}');")

        js_code = f"""
        (async () => {{
            try {{
                const b64 = window._fileChunks.join('');
                window._fileChunks = [];
                
                const res = await fetch(`data:{mime_type};base64,${{b64}}`);
                const blob = await res.blob();
                const file = new File([blob], "{path.name}", {{ type: "{mime_type}" }});
                ...
                const target = document.querySelector('.ql-editor, div[contenteditable="true"]');
                if (target) {{
                    target.focus();
    ...
    async def send_message(self, message: str, files: list[str] = None) -> bool:
        if files and self.config.guest_mode:
            logger.error("✗ Chế độ Khách (Guest Mode) không hỗ trợ tải file. Vui lòng tắt guest_mode trong config.")
            return False
        ...
        # Tự động xóa nội dung cũ trong khung nhập (nếu có)
        try:
        ...
        if files and isinstance(files, list):
            logger.info("→ Bắt đầu xử lý nạp file vào khung chat...")
            for file_path in files:
                pasted = await self._paste_file(file_path)
                if pasted:
                    await asyncio.sleep(0.5)
            
            logger.info("⏳ Đang chờ hệ thống của Google xử lý tải file lên...")
            await asyncio.sleep(3.0)
            
            js_check_upload = """
            (() => {
                const spinners = document.querySelectorAll('mat-progress-spinner, [role="progressbar"], circle[stroke-dashoffset]');
                ...
                const sendBtn = document.querySelector('button[aria-label*="Send"], button[aria-label*="Gửi"]');
            ...
            consecutive_done = 0
            for _ in range(600):
                is_uploading = await self.page.evaluate(js_check_upload)
                ...
                if consecutive_done >= 6:
                    break
                    
                await asyncio.sleep(0.5)
                
            await asyncio.sleep(1.0)
            logger.info("✓ Tải file hoàn tất!")
        ...
        logger.info("→ Gửi nội dung...")
        js_click = """
        (() => {
            const btns = document.querySelectorAll('button[aria-label*="Send"], button[aria-label*="Gửi"]');
            for (let b of btns) {
        ...
        """
        if not await self.page.evaluate(js_click):
            send_btn = await self._find_send_button()
            if send_btn:
                try: await send_btn.click()
                except Exception: await input_area.send_keys("\n")
            else:
    ...
    # ----------------------------------------------------------
    # Các hàm Detect Text và Wait Response (Đã Fix Ảo Giác Giao Diện)
    # ----------------------------------------------------------
    ...
    (() => {
        function extractText(el) {
            ...
            // [FIX] Lọc bỏ hoàn toàn các nhãn tên rác do DOM lấy nhầm lúc đầu (Sandbox đang chạy)
            if (txt === "Gemini đã nói" || txt === "Gemini said" || txt === "") return null;
            
            return txt;
        }
        const modelResp = document.querySelectorAll('model-response .model-response-text, model-response message-content');
        if (modelResp.length) return extractText(modelResp[modelResp.length - 1]);
        const md = document.querySelectorAll('.response-container-content .markdown, .markdown-main-panel');
    ...
    (() => {
        ...
        const stop = document.querySelector('button[aria-label*="Stop"], button[aria-label*="Dừng"]');
        if (isVisible(stop)) return true;
        
        const prog = document.querySelector('mat-progress-bar, [role="progressbar"], .loading-indicator');
        if (isVisible(prog)) return true;
        
        const spinner = document.querySelector('mat-spinner, mat-progress-spinner, .gmat-mdc-circular-progress, circle[stroke-dashoffset], [class*="analyzing"]');
    ...
    async def _is_generating(self) -> bool:
        try: return bool(await self.page.evaluate(self._JS_IS_GENERATING))
        except Exception: return False
    ...
    async def wait_response(self, timeout: int | None = None) -> str | None:
        ...
        # [FIX] Mở rộng thời gian chờ tối thiểu lên 300 giây (5 phút).
        # Các file cực kỳ nặng cần cực nhiều thời gian Sandbox xử lý.
        if timeout < 300:
            timeout = 300
        ...
        while time.time() - start < timeout:
            ...
            # Nhờ code JS phía trên, nếu "text" chỉ là nhãn "Gemini đã nói", 
            # biến text ở đây sẽ là rỗng (None), script sẽ tiếp tục kiên nhẫn đợi ở block bên dưới.
            if text:
                ...
                if text == prev_text:
                    if not generating:
                        ...
                        long_stable += 1
                        # [FIX] Tăng mức đứt gánh an toàn lên 120 giây
                        if long_stable >= 120: 
                            self.last_bot_response = text.strip()
                            return text.strip()
                else:
    ...
    async def chat(self, message: str, files: list[str] = None) -> str | None:
        ok = await self.send_message(message, files=files)
    ...
    async def chat_stream(self, message: str, files: list[str] = None):
        """
